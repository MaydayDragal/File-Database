...
| Lines (layout) | `layoutLines(rows)` reads the table from where each WORD sat, because the engine returns a two-column row as one line about as often as two. The descriptions column begins at the right edge of the last heading before `INSTRUCTIONS` — not at that heading's own left edge, which is centred over a wide column and would clip every description. Every row is cut at that edge, rows with nothing in the column are dropped, and what remains is cut into lines on vertical gaps over 0.75 × the median row height. The op code is the rightmost code-shaped token between the `LINE` heading and the descriptions, skipping the line letter and `CC`; `CW` and `CV` are op codes on these ROs, not pay types |
