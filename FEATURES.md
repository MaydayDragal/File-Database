...
| Lines (layout) | `layoutLines(rows)` reads the table from where rows sat on the page: the column edge is the widest horizontal gap left of the `INSTRUCTIONS` heading (not the heading's own left edge, which moves with how the engine groups that row), rows below it in that column are cut into lines on vertical gaps over 0.75 × the median row height, and the `LINE`/`OP CODE` cells are read from the column beside them |
