...
| Messages as function calls | Navigation, keyboard commands, restores, the RO's "select in Files" mode: a feature calls `shell.send({type, …})`, the shell calls `instance.receive({type, …})` — the message contract the per-app pages once spoke over `postMessage` (§9) | Shell and feature instances |
...
`markup.js` (its former `<body>`) and `styles.css`; Files, LI Documents, Tool
Inventory and Repair Orders also have `search.js`, their quick-open provider,
which the registry ([src/features/index.js](src/features/index.js)) loads
without the feature's UI. The shell's own modules are
[src/shell/index.js](src/shell/index.js) and the vehicle view
[src/shell/vehicle.js](src/shell/vehicle.js); dialogs any feature can await
live in [src/ui/](src/ui/) (the duplicate review, the migration dialog). [viewer.html](viewer.html)
still exists as a standalone host of the Extract feature — loading it as one
classic script, `viewer.js` (generated by [tools/build-viewer.mjs](tools/build-viewer.mjs)),
because a browser refuses a module from `file://` — so a copy of it next to a
backup can always get the files back out; the other per-app pages are gone
...
## 9. The data layer, messages and storage
...
Features use the data layer through its ES-module facade
([src/data/index.js](src/data/index.js): `db`, `bus`, `repos`, `jobs`, `intake`,
`backup`, `migrate`, `boot`) or the same objects on `window.FDData`.

**Every repo** (`repos.<store>`) has the same verbs: `get(key)`, `getMany(keys)`,
`list()`, `listBy(index, value)`, `keys()`, `count(index?, value?)`,
`each(cb)`, `put(rec, { expectedRev })`, `putMany(recs)`, `remove(key)`,
`removeMany(keys)`, `clear()`. Each write bumps `rev`; a write with
`expectedRev` is refused with `RevConflict` (carrying the current record) when
someone else wrote first; a promise settles on the transaction's completion,
never on a request's success; after each commit the bus announces
`<store>:changed` with the keys. Repos key by their store's own keyPath.

| Repo | Beyond the common verbs |
| --- | --- |
| `files` (+ `blobs`, `thumbs`) | `listMeta({ inFiles, trash })` (metadata + thumbnail, never bytes; the trash left out unless `trash: "with" \| "only"`), `getFull`, `getBlob`, `getBlobRecord`, `getThumb`, `setThumb` (no `rev` bump), `putFull(rec, { verify, reuse })` (hashes, writes files/blobs/thumbs in one transaction, re-reads and compares when `verify`; `reuse` shares another record's bytes as `blobId`), `update(id, patch)` (metadata only), `ingest(entries)` (per-file results), `trash(ids)`, `restore(ids)`, `listTrash()`, `purge(ids)` → `{ purged, blocked }` (refuses anything a link or an LI document still references), `emptyTrash()`, `referencesOf(id)`, `removeFull(ids)` (the unconditional primitive under purge; a blobs row goes only when no record names it), `eachFull(cb, opts)`, `bySha256(digest)`, `findDuplicates(blobs)`, `byVin`, `byCollection`, `byDocId`, `renameCollection`, `countInFiles()`, `classify(file)`, `buildSearchText(rec)`, `blobKey(rec)` |
| `documents` | `putWithFile(doc, blob, { oldId })` (the document and its PDF as a `files` record, `inFiles 0`, in one transaction; `oldId` re-keys), `getBlob(id)`, `removeWithFile(id)` / `removeManyWithFile(ids)` (its PDF goes too unless it was added to Files), `byLi(li)`, `latestOf(li)`, `verNum(doc)` |
| `tools`, `photos` | The common verbs |
| `ros` | `validate` enforces the unique `roKey` (D7, `RoConflict` naming the other record — trashed or not); `byNumber`, `byVin`, `live()`, `listTrash()`; attachments `attach(roId, fileIds)`, `detach`, `attachments(roId, { trash })`, `ofFile(fileId)`; pins `pinDocument(roId, doc)` (keeps the LI number and version on the link), `unpinDocument`, `pinTool`, `unpinTool`, `references(roId)` → `{ documents: [{ link, doc, latest, newer }], tools: [{ link, tool }] }`; `trash(id, { attachments: "keep" \| "unlink" \| "trash" })` → `{ trashed, unlinked, kept }`, `restore(id, { files })` (brings back only files its own delete trashed, marked `trashedWith`), `purge(id)` → `{ purged, refs }` (its own links go, its files stay) |
| `links` | Typed relations `{ fromType, fromId, toType, toId, kind }` keyed by all five: `link`, `linkMany`, `unlink`, `from(type, id, kind?)`, `to(type, id, kind?)`, `removeFor(type, id)`, `idOf(…)`. Kinds today: `ro → file attachment`, `ro → document reference`, `ro → tool required-tool` |
| `vehicles` | Keyed by VIN: `note(vin, { source, fin, model })` (upsert, never a downgrade; records the check-digit verdict), `confirm(vin, yes)` (a technician's confirmation), `summary(vin, { max })` → the record, live files and repair orders, and the LI documents and tools that fit its model series, from one transaction |
| `jobs` | The store behind `FDData.jobs` (below) |
| `settings` | `getValue(key, fallback)`, `setValue(key, value)`, `removeKey`, `listPrefix(prefix)` |
| `log` | Reserved for the logger |

| Module | API |
| --- | --- |
| `db.js` | `open()` (the live generation), `openNamed(name)`, `run(stores, mode, fn, name?)` — the one commit boundary every repo uses; `currentName`, `nextName`, `setCurrentName`, `probe(name)` (never creates), `withExisting`, `readAll`, `countIn`, `deleteDatabase`, `closeAll` |
| `bus.js` | `emit(topic, detail, { local })`, `on(topic, fn)` → unsubscribe; an `EventTarget` mirrored over `BroadcastChannel("file-database")`; `detail.remote` marks events from another context |
| `jobs.js` | `enqueue(type, inputIds, meta)`, `register(type, runner)` (runs that queue in this page under `navigator.locks`), `whenDone(id, ms)`, `get`, `listActive`, `cancel`, `kick`, `requeueStale`, `sweep`; heartbeat, retry up to 5 |
| `intake.js` | `ingest(target, files, { collection, vin, roId, reviewDuplicates, … })`: eager read, the duplicate review before anything is written (reuse / keep / skip; a skipped duplicate's stored file comes out of the trash and is attached to `roId`), store with verification, per-file results, then the follow-up jobs (`thumb` + `vin-detect` for Files, `li-import`, `toolbox-intake`) and an RO `attachment` link when `roId` is given; `classifyTarget(file)`, `looksLikeLI`, `materialize` |
| `backup.js` | `collect({ generation })` → `{ blob, meta }`: one `.fdb` ([src/core/formats/fdb.js](src/core/formats/fdb.js), magic `FDBK`) read in a single transaction; `restore(file, { onProgress })` into a new generation, verified (counts, a payload behind every file record, every blob's hash), then the pointer flips and the old generation is deleted; `fileName()`, `countsOf(name)` |
| `migrate.js` | `status()` / `run()` / `deleteLegacy()`: `file-vault`, `LIDocsDB`, `tool-inventory`, `repair-orders` → one generation; anything already in the live generation is carried in first; files in an RO's collection become links; colliding RO numbers keep the older record's key |
| `boot.js` | `FDData.boot()`: the first call — migrate if needed, open the live generation, requeue jobs a dead page left running |
...
The contract the per-app pages once spoke over `postMessage`, kept as function
...
The shell queues messages until the feature has mounted; Vault/LI/Inventory
also queue their navigation messages until their data has loaded. Message names alone are not authentication;
...
| `log` | (reserved) | Unused: the logger keeps its own `fv-debug` database (below), so it can record errors from before the data layer opens |
...
the latest worker and reloads once — at most once a minute per tab, after which
the shell tells the user to hard-reload instead of looping. The worker
...
build: it is served as it is. The one build step is the portable package:
[build-portable.mjs](tools/build-portable.mjs) deletes and recreates
`dist-portable/`, copies the runtime assets, builds the tool catalog, and
bundles `src/main.js` with every module it imports (the features and their
...
bundle, the worker's precache lists, vendored PDF.js), the unit tests
(`tests/unit/`, incl. the data layer and the goldens) and the LI-number,
binary-backup and Blob-integrity checks — Node only, seconds. `npm test` runs
...
| `e2e-outdated.mjs` | An out-of-date copy of the app meeting a newer database: the app cache cleared (the OCR cache kept), exactly one reload, then a hard-reload message instead of a loop; the database untouched |
...
[qa.yml](.github/workflows/qa.yml) runs three gates on Node 20: `checks`
(dependency audit and `npm run test:fast`) on every push and pull request,
`qa` (the whole browser suite) on pull requests, and `portable` (the package
built, exercised from `file://` and uploaded as an artifact) on pushes to the
live branch. Mercedes-prefix VIN validation, once a browser suite, is pinned by
the VIN golden unit test. [zip-test.yml](.github/workflows/zip-test.yml) independently checks
