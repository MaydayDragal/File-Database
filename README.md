...
- **Group by VIN** — **⋮ → Scan files for VINs** reads every file for a 17‑character vehicle identification number: filenames and text files directly, PDFs through their text layer, and images or scanned PDFs via OCR (loaded online once, like the LI app's OCR). Detected VINs appear in the sidebar and in the **🚗 By VIN** view, which groups files under one header per vehicle; each file's VIN is also editable in its detail panel (with a per‑file **Detect** button).
