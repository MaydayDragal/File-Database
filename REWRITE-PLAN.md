...
Every feature module has the same four optional exports, and the shell only ever
calls those:
...
| --- | --- | --- | --- |
| `files` | `id` | name, type, kind, size, tags[], collection, note, starred, createdAt, updatedAt, vins[], fins[], vinScan, srcMtime, **roId?**, **docId?**, searchText | kind, collection, starred, updatedAt, tags*, vins*, roId, docId |
| `blobs` | `id` (= file id) | blob | — |
| `thumbs` | `id` (= file id) | blob | — |
| `documents` | `id` | li, ver, title, reason, fgroup, date, validity, text, pages, **fileId**, filename, size, mtime, noText, err, star, added | li, fgroup, star, added |
| `tools` | `id` | as today (`inventory/app.js normalize()`) | toolNo, grp |
| `photos` | `id` | blob | — |
| `ros` | `id` | ro, vehicle, vin, tag, mileage, color, opened, customer, advisor, phone, email, lines[], scanText, createdAt, updatedAt | ro, vin, updatedAt |
...
- **RO ↔ files is a foreign key** (`files.roId`), not a collection string. Renaming
  an RO touches one record. RO numbers no longer need to be unique.
...
- **A vehicle is a query**, not a message: `files.byVin(v)`, `ros.byVin(v)`,
  `documents.byModel(series)` off the indexes. A `vehicles` store is *not* proposed
  until there is something to store per vehicle (notes, aliases) — YAGNI.
- **One backup.** `backup.collect()` walks every feature's `backup` export into one
  ...
  strict validator in `vault/backup-format.js` and its tests carry over). RO records
  are finally included. `.fvault`/`.lidb`/`.tidb` keep importing; Extract reads all four.
...
3. Verify counts store-by-store and record `settings.migration = { from, counts, at }`.
...
- Merge the two folder-sync implementations (Vault `syncDir`, LI `resumeSyncDir`)
  into `services/folder-sync.js` with a per-feature `onFiles` callback.
...
- **RO ↔ files by id**: rename an RO without touching files; delete asks whether
  to keep, unlink or delete attachments.
- **One backup file** (`.fdb`) with ROs included; "Back up everything" is one
  download and one restore. Legacy importers stay.
- **Vehicle view across everything**: pin a VIN and the shell shows files, ROs and
  model-scoped LI/tool counts from one query; `#vehicle/<VIN>` route.
- **Quick-open across all stores** from the indexes (LI number, tool number, VIN,
  RO number, filename) with results, not "search app X for…".
...
- **A `vehicles` store now.** Indexes answer every vehicle question today.
