/* Extract (the Database File Viewer) — as a feature module (REWRITE-PLAN.md Phase 4). */
...
export function start(root, host, shell) {
    ...
    return FDCore.formats.fdb.parse(file).then(function (r) {
      ...
      (recs.files || []).forEach(function (f) {
        ...
        var path = d
          ...
          : (f.deletedAt ? "Trash/" : "Files/") + (f.collection ? safeName(f.collection, "collection") + "/" : "") + safeName(f.name, "file");
  ...
  // A backup dropped anywhere while this tab is showing opens here (the shell
  // asks the feature on screen before routing a drop); anything else goes to
  // the platform's unified intake.
