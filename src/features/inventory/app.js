/* Tool Inventory — the app, as a feature module (REWRITE-PLAN.md Phase 4). */
/*
...
export function start(root, host, shell) {
  ...
  function toast(m) {
    ...
    if (embedded) { try { shell.send({ type: "shell-toast", app: "inventory", msg: String(m) }); } catch (e) {} }
  ...
  function crossNav(app, payload, hashPath) {
    if (embedded) { try { shell.send({ type: "shell-nav", app: app, payload: payload }); return; } catch (e) {} }
  ...
  function wire() {
    var st;
    $("#search").addEventListener("input", function (e) { var input = e.currentTarget; clearTimeout(st); st = setTimeout(function () { state.q = input.value.trim(); apply(); }, 130); });
    ...
    document.addEventListener("click", function (e) { var t = e.composedPath ? e.composedPath()[0] : e.target; if (!mn.hidden && !mn.contains(t) && t !== mb) mn.hidden = true; });
    ...
    // (A .tidb dropped anywhere lands here through the shell's router.)
    ...
    root.addEventListener("keydown", function (e) {
      ...
      else if (e.key === "/" && active && active.tagName !== "INPUT" && active.tagName !== "TEXTAREA") { e.preventDefault(); $("#search").focus(); }
    });
  ...
  // (next to index.html). If the database is empty and that file exists,
  // offer a one-click load.
