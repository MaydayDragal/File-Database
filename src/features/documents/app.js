...
export function start(root, host, shell) {
  ...
  function toast(msg) {
    ...
    try { if (true) shell.send({ type: "shell-toast", app: "li", msg: String(msg) }); } catch (e) {}
  ...
  function refresh() { return allDocs().then(function (d) { docs = d; applyView(); updateStore(); try { if (true) shell.send({ type: "li-changed" }); } catch (e) {} }); }
  ...
  function crossNav(app, payload, hashPath) {
    var emb = false; try { emb = true; } catch (e) { emb = true; }
    if (emb) { try { shell.send({ type: "shell-nav", app: app, payload: payload }); return; } catch (x) {} }
  ...
  function renderSide(pdf, col, seq) {
    ...
    if (!cw) { var mo = root.querySelector("#diffOverlay .modal"); cw = mo && mo.clientWidth ? Math.max(mo.clientWidth / 2 - 40, 300) : 620; }
  ...
  function setHash(li) { shell.setSub("li", li || ""); }
  // (Deep links arrive from the shell as li-open messages.)
  ...
  $("dCopyLink").addEventListener("click", function () {
    ...
    copyText(location.href.split("#")[0] + "#li/" + encodeURIComponent(li), "Link copied.");
  });
  ...
  root.querySelectorAll(".overlay").forEach(function (o) { o.addEventListener("click", function (e) { if (e.target !== o) return; if (o.id === "diffOverlay") closeDiff(); else closeOverlays(); }); });
  root.addEventListener("keydown", function (e) { if (e.key !== "Escape") return; if ($("diffOverlay").classList.contains("show")) closeDiff(); else closeOverlays(); });

  // (Offline cache and install belong to the shell now.)
  ...
  function updInstallBtn() { var hide = true; $("installBtn").style.display = hide ? "none" : ""; $("menuSepInstall").style.display = hide ? "none" : ""; }
  updInstallBtn();
  ...
  document.addEventListener("click", function (e) { var t = e.composedPath ? e.composedPath()[0] : e.target; if (!$("appMenu").hidden && !$("appMenu").contains(t) && t !== $("menuBtn")) closeMenu(); });
  root.addEventListener("keydown", function (e) { if (e.key === "Escape" && !$("appMenu").hidden) closeMenu(); });
  ...
  (function () {
    ...
    if (back && embedded) back.addEventListener("click", function (e) {
      e.preventDefault();
      try { shell.send({ type: "vault-nav", to: "files" }); } catch (x) {}
    });
    // (Drops over the panel land in the shell's one router.)
  })();
