/* Files (the File Vault) — the app, as a feature module (REWRITE-PLAN.md Phase 4).
 * start(root, host, shell) wires the panel's shadow root; `shell.send` carries the
 * messages this app used to post to its parent frame. */
/*
...
export function start(root, host, shell) {
  ...
  // Always hosted by the platform shell now (one page, REWRITE-PLAN.md Phase 4).
  const embedded = true;
  ...
  function toast(msg, actionLabel, actionFn) {
    ...
    if (embedded) { try { shell.send({ type: "shell-toast", app: "vault", msg: String(msg) }); } catch (e) {} }
    const el = $("#toast");
    el.innerHTML = "";
  ...
  function refreshCardThumb(id, thumb) {
    const box = root.querySelector('.card[data-id="' + (window.CSS && CSS.escape ? CSS.escape(id) : id) + '"] .card__thumb');
    if (!box) return;
      ...
      Array.from(groups.keys()).sort().forEach((v) => {
        ...
        head.querySelectorAll(".group-head__link").forEach((b) => {
          b.onclick = (e) => {
            e.stopPropagation();
            if (b.dataset.pin) {
              try { shell.send({ type: "shell-pin-vehicle", vin: b.dataset.pin }); } catch (x) {}
              return;
            }
  ...
  function wireBulkBar() {
    ...
    $("#bulk-send-li").onclick = async () => {
      ...
      if (embedded && sent) { try { shell.send({ type: "shell-nav", app: "li" }); } catch (e) {} }
    };
    $("#bulk-send-toolbox").onclick = async () => {
      ...
      if (embedded && sent) { try { shell.send({ type: "shell-nav", app: "toolbox", tab }); } catch (e) {} }
    };
  ...
  function crossNav(app, payload, hashPath) {
    if (embedded) {
      try { shell.send({ type: "shell-nav", app, payload }); return; } catch (e) {}
    }
  ...
  async function sendCurrentToLI() {
    ...
    try {
      ...
      if (embedded) {
        try { shell.send({ type: "shell-nav", app: "li" }); } catch (e) {}
      }
  ...
  async function sendCurrentToToolbox() {
    ...
    try {
      ...
      if (embedded) {
        try { shell.send({ type: "shell-nav", app: "toolbox", tab }); } catch (e) {}
      }
      closeDetail();
  ...
  // ---------- Theme ----------
  // The shell owns the theme (src/styles/tokens.css); the vault's own toggle is hidden.
  ...
  function intake(fileList) {
    ...
    if (embedded) {
      ...
      const collection = state.filter.startsWith("collection:") ? state.filter.slice(11) : "";
      try { shell.send({ type: "shell-add-files", files: files, collection: collection }); return; } catch (e) {}
    }
    addFiles(files);
  }
  // The shell's window-level drop zone and paste handler route files; the
  // collection that is open here rides along so a drop into "Taxes 2026"
  // still files there.
  function initDnD() {}
  ...
  function wire() {
    ...
    $("#empty-add").onclick = () => {
      if (embedded) { try { shell.send({ type: "shell-open-picker" }); return; } catch (e) {} }
      $("#file-input").click();
    };
    ...
    $("#search-input").addEventListener("input", (e) => {
      // Read the input, not the event's target: once dispatch is over an
      // event from inside a shadow root reports the host as its target.
      const input = e.currentTarget;
      clearTimeout(searchTimer);
      searchTimer = setTimeout(() => { state.query = input.value.trim(); render(); }, 140);
    });
    ...
    document.addEventListener("click", (e) => {
      ...
      if (!moreMenu.hidden && !moreMenu.contains(t) && t !== moreBtn) moreMenu.hidden = true;
    });
    ...
    // Keyboard shortcuts
    // (Alt+N and Ctrl+K are the shell's; events from in here reach it too.)
    root.addEventListener("keydown", (e) => {
      ...
      if (e.key === "/") { e.preventDefault(); $("#search-input").focus(); }
        ...
        if (embedded) { try { shell.send({ type: "shell-open-picker" }); } catch (x) {} }
  ...
  function handleMenu(action, e) {
    if (action === "export") exportVault();
      ...
      if (embedded) { try { shell.send({ type: "shell-nav", app: "viewer" }); } catch (x) {} }
  ...
  async function importSelectionToRo() {
    ...
    if (embedded) { try { shell.send({ type: "shell-nav", app: "ros", payload: { type: "shell-nav", id: target.roId } }); } catch (e) {} }
  }
