/* Toolbox — tab switching and the File Database platform integration, as a
 * feature module (REWRITE-PLAN.md Phase 4). The ten tools are ./tools/*.js. */
export function start(root, host, shell) {
  "use strict";

/* ===== Tab switching ===== */
  var tabs = root.querySelectorAll('.tab');
  var tools = root.querySelectorAll('.tool');
  ...
  window.__openTab = function (key) {
    ...
    var btn = root.querySelector('.tab[data-tab="' + key + '"]');
    if (btn) btn.click();
  };
  // (Deep links arrive from the shell as toolbox-open messages.)

/* ===== File Database platform integration ===== */
  // Messages from the shell: open a tool tab.
  function receive(d) {
    if (!d || typeof d !== "object") return;
    if (d.type === "toolbox-open" && d.tab) { if (window.__openTab) window.__openTab(d.tab); }
  }
  ...
  window.__vaultOffer = function (blob, name) {
    try {
      ...
      root.appendChild(snack); // inside the panel, as it was inside the frame
      ...
      Object.keys(groups).forEach(function (tab, i) {
        ...
        try {
          ...
          if (tab === "media" && window.__mediaAddFiles) { window.__mediaAddFiles(files); }
            ...
            var input = root.getElementById(INPUTS[tab]);
